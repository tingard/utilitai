...
class TestAdd:
    ...
    def test_returns_function_unchanged(self, things: ToConsider[Context]):
        ...
        assert things.add("named")(score) is score
        assert things.add(score) is score
    ...
    def test_rejects_duplicate_names(self, things: ToConsider[Context]):
        things.add_constant("thing", 1.0)
        with pytest.raises(ValueError, match="already been added"):
            things.add_constant("thing", 2.0)
...
class TestConsider:
    ...
    def test_ties_favour_the_first_added_option(self, things: ToConsider[Context]):
        things.add_constant("first", 1.0)
        things.add_constant("second", 1.0)
        assert things.consider(Context()) == "first"

    def test_handles_negative_scores(self, things: ToConsider[Context]):
        things.add_constant("bad", -10.0)
        things.add_constant("less bad", -1.0)
    ...
    def test_passes_the_context_through(self, things: ToConsider[Context]):
        seen: list[Context] = []
        things.add("watcher")(lambda ctx: seen.append(ctx) or 1.0)
...
class TestScore:
    def test_scores_every_option_in_order(self, things: ToConsider[Context]):
        things.add_constant("a", 0.1)
        things.add_constant("b", 0.2)
        assert list(things.score(Context()).items()) == [("a", 0.1), ("b", 0.2)]
...
class TestContainer:
    ...
    def test_iterates_over_names(self, things: ToConsider[Context]):
        things.add_constant("a", 1.0)
        things.add_constant("b", 1.0)
        assert list(things) == ["a", "b"]
